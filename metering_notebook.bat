...
if is_interactive():
    import subprocess
    subprocess.call(['jupyter', 'nbconvert', '--to', 'script', 'metering_notebook.ipynb'])
    try:
        os.remove('metering_notebook.bat')
    except: pass
    os.rename('metering_notebook.py', 'metering_notebook.bat')
...
ins1, ins2, ins3, ins4, ins5, ins6 = get_instruments('COM10')
inst = inputs_writer()
prev = -9999999
prev_save = -9999999
